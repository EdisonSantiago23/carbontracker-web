...
const Index = () => {
  ...
  const confir = () => {
    ...
    if (elm) {
      FirestoreService.deleteBomba(elm).then(docRef => {
        setOpen(false);
      });
    }
  };
  ...
  return loading ? <LoadingData onLoad={() => borrarSearch()} /> : <Paper className={classes.root}>
      ...
      {usuario ? <Box mt={2}>
          <SearchBar value={searched} onFocus={() => borrarSearch()} onChange={searchVal => requestSearch(searchVal)} onCancelSearch={() => cancelSearch()} />
              ...
              return <StyledTableRow hover tabIndex={-1} key={index}>
                            ...
                            <StyledTableCell align="center">
                              <Tooltip title="Habitaciones">
                                <Link to={"/administrador/hospitales/habitaciones/" + row.id}>
                              ...
                              <Tooltip title="Jefes">
                                <Link to={"/administrador/hospitales/Jefes/" + row.id}>
