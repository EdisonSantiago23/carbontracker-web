...
const CreateForm = ({ className, send, data, info, areaId }) => {
  ...
  const handleSubmitForm = async (values) => {
    ...
    try {
      if (data) {
        await FirestoreService.updateHabitacion(values, threadKey, data.id, areaId);
        enqueueSnackbar("Habitación actualizada correctamente", { variant: "success" });
      } else {
        await FirestoreService.newHabitacion(values, threadKey, areaId);
        enqueueSnackbar("Habitación creada correctamente", { variant: "success" });
      }
      send();
    } catch (err) {
      console.error(err);
      enqueueSnackbar("Ocurrió un error al guardar la habitación", { variant: "error" });
    } finally {
      setLoading(false);
    }
  };
