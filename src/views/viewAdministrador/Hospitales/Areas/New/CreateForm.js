...
const CreateForm = ({
  ...
  const agregar = values => {
    ...
    try {
      FirestoreService.newArea(values, threadKey).then(() => {
        setLoading(false);
  ...
  const update = values => {
    FirestoreService.updateArea(values, threadKey, data.id).then(rss => {
      setLoading(false);
      ...
      values
          ...
          {loading ? <Box display="flex" justifyContent="center" my={5}>
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} lg={6}>
                  <TextField error={errors.Numero && touched.Numero} helperText={errors.Numero && touched.Numero && errors.Numero} label="Numero" name="Numero" placeholder="Numero" variant="outlined" fullWidth="true" value={values.Numero} onChange={handleChange} onBlur={handleBlur} />
                </Grid>
              </Grid>
