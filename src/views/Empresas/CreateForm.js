...
const CreateForm = ({ className, send, data }) => {
  ...
  const agregar = async (values) => {
      ...
      HospitalService.newHospital(values).then(() => {
        setLoading(false);
    ...
    const agregar2 = async (values) => {
    await HospitalService.subirImagenes(imageAsFile).then((response) => {
      values.logoImg=response;
  ...
  const update = async (values) => {
   await HospitalService.subirImagenes(imageAsFile).then((response) => {
      values.logoImg=response
      HospitalService.updateHospital(values, data.id).then((rss) => {
        setLoading(false);
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} lg={6}>
                  <TextField
                    ...
                    label="Direccion"
                    name="Direccion"
                    placeholder="Direccion del hospital"
                    variant="outlined"
                    fullWidth
                ...
                <Grid item xs={12} lg={6}>
                  <TextField
                    ...
                    label="Detalle"
                    name="Detalle"
                    placeholder="Detalle del hospital"
                    variant="outlined"
                    fullWidth
