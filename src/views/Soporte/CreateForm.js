...
const CreateForm = ({ className, send, data }) => {
  ...
  const agregar = (values) => {
    try {
      SoporteService.newSoporte(values).then(() => {
        setLoading(false);
  ...
  const update = (values) => {
    SoporteService.updateSoporte(values, data.id).then((rss) => {
      setLoading(false);
    ...
    <Formik
      ...
      validationSchema={Yup.object().shape({
        Nombre: Yup.string()
          ...
          Correo: Yup.string()
          .required("¡Se requiere rellenar eleste campo!")
          .test("", "Campo Correo se encuentra vacio", function vnombre(
            nombre
          ) {
            if (typeof nombre === "undefined") {
              return false;
        ...
        Direccion: Yup.string()
          ...
          .test("", "Campo Direccion se encuentra vacio", function vdireccion(
            direccion
          ) {
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} lg={6}>
                  <TextField
                    ...
                    label="Direccion"
                    name="Direccion"
                    placeholder="Direccion del Soporte"
                    variant="outlined"
                    fullWidth
                ...
                <Grid item xs={12} lg={6}>
                  <TextField
                    ...
                    label="Detalle"
                    name="Detalle"
                    placeholder="Detalle del Soporte"
                    variant="outlined"
                    fullWidth
                ...
                <Grid item xs={12} lg={6}>
                  <TextField
                    ...
                    helperText={
                      errors.Correo && touched.DeCorreotalle && errors.Correo
                    }
                    label="Correo"
                    name="Correo"
                    placeholder="Correo del Soporte"
                    variant="outlined"
                    fullWidth
                    value={values.Correo}
                    onChange={handleChange}
